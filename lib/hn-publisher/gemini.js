...
const METADATA_MAX_CHARACTERS = 50000
...
async function translateMetadata(article, story, geminiOptions) {
  const articleText = article.blocks
    .map(getBlockText)
    .join('\n')
    .slice(0, METADATA_MAX_CHARACTERS)
  const prompt = [
    '根据以下英文文章生成博客元数据。',
    '返回严格 JSON：{"title":"简洁的中文新闻标题","summary":"2到3句简体中文摘要"}。',
    '标题应根据全文提炼核心事件或结论，约15到30个中文字符，不要照译原标题，不要标题党。',
    '标题不要添加“HN译文”等前缀，不要使用 Markdown、引号、换行或 emoji。摘要不得声称读过未提供的内容。',
    `原始标题：${article.sourceTitle || story.title}`,
    `完整正文（最多${METADATA_MAX_CHARACTERS}字符）：${articleText}`
  ].join('\n')
